...
description: "Submit Alibaba Cloud AISC Skill file security scans, poll scan tasks, diagnose API or upload failures, interpret scan reports, and generate readable Markdown reports with optional AI translation for malicious code, prompt injection, hardcoded credentials, sensitive data, risky configuration, and other Skill package security findings. Use when the user asks whether a Skill package or Skill file is safe, wants to scan/check/detect a Skill package, provides one or more Skill download URLs for AISC security detection, asks to run or poll CreateSkillFileCheck/ListSubTasks, asks about check-report.json or rootTaskId status, asks to turn the raw JSON into a readable report or translate findings through OpenAI/Anthropic/custom model APIs, needs help with permission/parameter/throttling/system scan errors, or needs to choose between multiple candidate Skill files for security scanning. Trigger phrases include AISC scan, Skill security check, Skill file check, Skill report generation, Skill 安全检测, Skill 文件安全扫描, 生成检测报告, 翻译风险描述, 扫描 Skill 包, 检测 Skill 是否安全, and 检查 Skill 文件有没有恶意代码/敏感凭据/prompt 注入."
---
...
When the user asks to turn a raw AISC JSON report into a readable report, use [generate_report.py](generate_report.py):

```bash
python3 generate_report.py ./check-report.json --output-dir ./reports
```

The generator accepts the `tasks` at the top level, `poll.tasks`, and the nested `batches[].poll.tasks` shape produced by multi-batch `run`. It renders `Virus`, `Sensitive`, `Guardrail`, and `Config` findings, keeps failed-task status visible, and sanitizes common credential patterns in Markdown snippets. Sensitive detection values are always replaced with `[已脱敏]`; use `--hide-content` to hide all snippets. A poll-only JSON may not contain the original `file_name`; the generator never guesses from or hardcodes an internal hash, and uses `skill-task-<task-id>` instead. Supply `--file-names` with a JSON array or object, or generate the report from a `run` result that retains `submit.upload_results.file_name`.

Translation is configured explicitly in the skill-local `.env` or through CLI options. The minimal `.env` configuration uses only `MODEL`, `API_URL`, and `API_KEY`; the generator infers OpenAI-compatible or Anthropic-compatible protocol from the API URL. The older `AISCC_REPORT_*` variables remain supported for advanced setups. Do not put API keys in prompts, command arguments, source files, or reports. Only sanitized finding descriptions are sent to the model; matched code snippets and sensitive detection results are never sent. Translation failures keep the original descriptions by default; use `--translation-error fail` to make them fatal.

For OpenAI-compatible providers such as Alibaba Cloud Model Studio, `AISCC_REPORT_API_URL` may be either the provider base URL or the full `/chat/completions` endpoint; the generator normalizes a base URL before sending the request. For Anthropic-compatible providers, it similarly appends `/messages` when only a base URL is configured.

## Upstream Maintenance

This directory is a locally maintained copy of the official skill at [aliyun/alibabacloud-aiops-skills](https://github.com/aliyun/alibabacloud-aiops-skills/tree/master/skills/security/asc/alibabacloud-aisc-skill-inspection). The local copy contains repository-specific changes, including `README.md` and `generate_report.py`. When the upstream skill changes, fetch and compare the upstream `SKILL.md` and AISC wrapper, then merge the relevant behavior and API changes manually. Do not overwrite the whole local directory, because that would remove the local report generator and translation support. See [README.md](README.md) for the exact comparison commands.
...
- [generate_report.py](generate_report.py): local Markdown report generator with optional model translation.
